...
ENV SGX_DOWNLOAD_URL_BASE "https://download.01.org/intel-sgx/sgx-dcap/1.3.1/linux/distro/ubuntuServer18.04"
...
ENV LIBSGX_DCAP_QL                  libsgx-dcap-ql_1.3.101.3-bionic1_amd64.deb
ENV LIBSGX_DCAP_QL_DEV              libsgx-dcap-ql-dev_1.3.101.3-bionic1_amd64.deb
ENV LIBSGX_DCAP_DEFAULT_QPL         libsgx-dcap-default-qpl_1.3.101.3-bionic1_amd64.deb
ENV LIBSGX_DCAP_DEFAULT_QPL_DEV     libsgx-dcap-default-qpl-dev_1.3.101.3-bionic1_amd64.deb
...
ENV LIBSGX_DCAP_QL_URL                  "$SGX_DOWNLOAD_URL_BASE/$LIBSGX_DCAP_QL"
ENV LIBSGX_DCAP_QL_DEV_URL              "$SGX_DOWNLOAD_URL_BASE/$LIBSGX_DCAP_QL_DEV"
ENV LIBSGX_DCAP_DEFAULT_QPL_URL         "$SGX_DOWNLOAD_URL_BASE/$LIBSGX_DCAP_DEFAULT_QPL"
ENV LIBSGX_DCAP_DEFAULT_QPL_DEV_URL     "$SGX_DOWNLOAD_URL_BASE/$LIBSGX_DCAP_DEFAULT_QPL_DEV"
ENV LIBSGX_DCAP_DEFAULT_QPL_DEV_DBG_URL "$SGX_DOWNLOAD_URL_BASE/$LIBSGX_DCAP_DEFAULT_QPL_DEV_DBG"
...
RUN mkdir ~/sgx                                                               && \
    ...
    wget -O $LIBSGX_DCAP_QL               "$LIBSGX_DCAP_QL_URL"               && \
    wget -O $LIBSGX_DCAP_QL_DEV           "$LIBSGX_DCAP_QL_DEV_URL"           && \
    wget -O $LIBSGX_DCAP_DEFAULT_QPL      "$LIBSGX_DCAP_DEFAULT_QPL_URL"      && \
    wget -O $LIBSGX_DCAP_DEFAULT_QPL_DEV  "$LIBSGX_DCAP_DEFAULT_QPL_DEV_URL"  && \
    wget -O $LIBSGX_DCAP_DEFAULT_QPL_DEV_DBG "$LIBSGX_DCAP_DEFAULT_QPL_DEV_DBG_URL"

RUN cd ~/sgx                                  && \
    dpkg -i $LIBSGX_ENCLAVE_COMMON            && \
    dpkg -i $LIBSGX_ENCLAVE_COMMON_DEV        && \
    dpkg -i $LIBSGX_ENCLAVE_COMMON_DBGSYM     && \
    dpkg -i $LIBSGX_DCAP_QL                   && \
    dpkg -i $LIBSGX_DCAP_QL_DEV               && \
    dpkg -i $LIBSGX_DCAP_DEFAULT_QPL          && \
    dpkg -i $LIBSGX_DCAP_DEFAULT_QPL_DEV      && \
    dpkg -i $LIBSGX_DCAP_DEFAULT_QPL_DEV_DBG  && \
