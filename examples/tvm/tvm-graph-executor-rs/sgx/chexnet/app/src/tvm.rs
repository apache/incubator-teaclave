...
fn preprocess(img: image::DynamicImage) -> Vec<f32> {
    ...
    for pixel in img.pixels() {
        ...
        //];
        let tmp = [
            (tmp[0] as f32 / 255.0 - 0.485) / 0.229, // R
            (tmp[1] as f32 / 255.0 - 0.456) / 0.224, // G
            (tmp[2] as f32 / 255.0 - 0.406) / 0.225, // B
        ];
        for e in &tmp {
            pixels.push(*e);
        }
    ...
    println!("[+] pixels: {:?}, {:?}", pixels.len(), &pixels[0..64]);
    pixels
}

pub fn sgx_inference(eid: sgx_enclave_id_t) {
    // Atelectasis.00017877_001.png
    let img_bytes = include_bytes!("../00017877_001.png");
