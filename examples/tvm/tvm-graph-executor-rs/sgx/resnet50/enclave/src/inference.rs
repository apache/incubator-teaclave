...
fn build_in_executor() -> GraphExecutor<'static, 'static> {
    let graph = Graph::try_from(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/lib/graph.json"
    )))
    .unwrap();
    let params_bytes = include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/lib/graph.params"));
    let params: HashMap<String, TVMTensor> = tvm_graph_rt::load_param_dict(params_bytes)
        .unwrap()
        .into_iter()
        .map(|(k, v)| (k, v.to_owned()))
        .collect();

    println!("[+] graph and params ready");
    let mut exec = GraphExecutor::new(graph, &*SYSLIB).unwrap();
    println!("[+] get exec");
    exec.load_params(params);
...
fn run_inference_old(pixels: Vec<f32>) -> Vec<f32> {
    println!("[+] run_inference");
    let input = data_preprocess(pixels);
    println!("[+] processed_input");
    let mut executor = build_in_executor();
    println!("[+] get executor");
    executor.set_input("data", input);
    println!("[+] set input");
    executor.run();
    println!("[+] finish run");
    executor.get_output(0).unwrap().to_vec()
}