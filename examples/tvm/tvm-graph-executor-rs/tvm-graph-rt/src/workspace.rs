...
pub extern "C" fn TVMBackendAllocWorkspace(
    ...
    let g_global_data_ptr = rsgx_get_global_data();
    let layout_en = unsafe { (*g_global_data_ptr).layout_entry_num };
    let tcs_max = unsafe { (*g_global_data_ptr).tcs_max_num};
    //println!("TVMBackendAllocWorkspace: {:?}; layout_en: {:?}, tcs_max: {:?}", nbytes, layout_en, tcs_max);
