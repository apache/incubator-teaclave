...
pub struct NaiveBayesWorker {
    worker_id: u32,
    func_name: String,
    func_type: FunctionType,
    input: Option<NaiveBayesInput>,
}

struct NaiveBayesInput {
    /// Sample model data
...
impl NaiveBayesWorker {
    pub fn new() -> Self {
        NaiveBayesWorker {
            worker_id: 0,
            func_name: "naive_bayes".to_string(),
...
impl Worker for NaiveBayesWorker {
    ...
    fn prepare_input(
        ...
        let naive_bayes_reg_payload: NaiveBayesPayload = serde_json::from_str(&payload)
            .or_else(|_| Err(Error::from(ErrorKind::InvalidInputError)))?;

        let input = parse_input_to_matrix(
            &naive_bayes_reg_payload.input_model_data,
            naive_bayes_reg_payload.input_model_columns,
        )?;

        let target = parse_input_to_matrix(
            &naive_bayes_reg_payload.target_model_data,
            naive_bayes_reg_payload.target_model_columns,
        )?;
        self.input = Some(NaiveBayesInput {
    ...
    fn execute(&mut self, _context: WorkerContext) -> Result<String> {
        ...
        let predict = model
            .predict(&input.input_model_data)
            .map_err(|_| Error::from(ErrorKind::InvalidInputError))?;
